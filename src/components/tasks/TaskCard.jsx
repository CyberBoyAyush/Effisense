...
const TaskCard = ({ task, onEdit, onDelete, onToggleComplete }) => {
  return (
    <div className={`p-3 sm:p-4 md:p-6 bg-gray-800/40 backdrop-blur-sm border border-gray-700/50 rounded-xl 
      hover:border-orange-500/50 transition-all duration-300 group ${task.completed ? 'opacity-75' : ''}`}>
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
        <div className="flex items-start gap-3 flex-1 w-full">
          ...
          <div className="space-y-2 min-w-0 flex-1">
            <h3 className="text-base sm:text-lg font-semibold text-white group-hover:text-orange-400 transition-colors truncate">
              {task.title}
            </h3>
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          <button
            onClick={onEdit}
            className="flex-1 sm:flex-initial px-3 sm:px-4 py-2 text-sm bg-orange-600/20 text-orange-400 rounded-lg hover:bg-orange-600/30 
              transition-all duration-200 border border-orange-500/20"
          >
            Edit
          </button>
          <button
